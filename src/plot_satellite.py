...
class GPSPlotter(object):
    ...
    def plot_utm_and_compass_bearing(self, ax, utm, compass_bearing, blit=True, color='r', arrow_length=15, arrow_head_width=10):
    ...
    def plot_latlong_density(self, ax, latlongs, include_image=True, include_colorbar=False, **kwargs):
        xy = filter(lambda x: x is not None, [self.latlong_to_coordinate(latlong) for latlong in latlongs])
        xy = np.array(list(xy))
        ...
        gridsize = 20
        hb = ax.hexbin(xy[:, 0], xy[:, 1], gridsize=gridsize, **kwargs)
...
def read_latlongs(bag_name):
    topics = ['/odometry/filtered', '/gps/fix']
    pos = []
    latlongs = []
    with rosbag.Bag(bag_name) as bag:
        for topic, msg, _ in bag.read_messages(topics):
            if topic == topics[0]:
                ...
                latlongs.append([msg.latitude, msg.longitude])
    return pos, latlongs
...
#     utm = latlong_to_utm(latlong)


# %%
path = mpltPath.Path(coords_out)
inside = path.contains_points(coords_in)
